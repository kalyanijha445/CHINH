...
=======
>>>>>>> ab532e4884dc155f32e60aec0f68754d7ca19613
import os
import sqlite3
import random
import smtplib
...
import io
<<<<<<< HEAD
import uuid # For generating unique filenames for captured images
import re # For cleaning Aadhaar numbers
=======
import uuid
import re
>>>>>>> ab532e4884dc155f32e60aec0f68754d7ca19613
...
<<<<<<< HEAD
        recent_photo TEXT, dental_xray TEXT, aadhaar_card TEXT, blood_group_report TEXT, face_encoding TEXT,
=======
...
>>>>>>> ab532e4884dc155f32e60aec0f68754d7ca19613
    ...
    ''')
<<<<<<< HEAD
    # Add columns if they don't exist to prevent errors on restart
=======
>>>>>>> ab532e4884dc155f32e60aec0f68754d7ca19613
    try:
...
<<<<<<< HEAD
# **FIX:** Helper function to robustly clean Aadhaar numbers
=======
>>>>>>> ab532e4884dc155f32e60aec0f68754d7ca19613
...
<<<<<<< HEAD
        pdf.set_text_color(*COLOR_PURPLE) # Use accent color
=======
...
<<<<<<< HEAD
    ...
    # --- SUBJECT DETAILS (WITH PHOTO LAYOUT FIX) ---
=======
>>>>>>> ab532e4884dc155f32e60aec0f68754d7ca19613
...
<<<<<<< HEAD
    ...
    # We use a conditional layout: one for when a photo is present, another for when it's not.
    if photo_path_to_draw:
        # --- Photo-Present Layout (Multi-column) ---
        y_start_details = pdf.get_y()
        photo_height = 60 # Define photo height in mm
        
        # Draw photo on the right side first
        pdf.image(photo_path_to_draw, x=140, y=y_start_details, w=50) # Set width to 50mm
        photo_end_y = y_start_details + photo_height
        
        # --- Draw text details in two columns on the left of the photo ---
=======
...
<<<<<<< HEAD
        ...
        # Define current Y for each line
        ...
        pdf.set_y(current_y + 8) # Move down for next row
...
<<<<<<< HEAD
        ...
        # Handle potentially long fields separately below the columns but above the photo's end
        add_single_row('Distinctive Marks:', report_data['identity_marks'])
        add_single_row('Clothing Worn:', report_data['clothing_description'])

        # Set cursor to the bottom of the tallest element in this section (either text or photo)
        final_y = max(pdf.get_y(), photo_end_y)
        pdf.set_y(final_y)

    else:
        # --- No-Photo Layout (Full width, cleaner) ---
=======
        current_y = pdf.get_y()
        add_detail_in_column('Aadhaar:', report_data['aadhaar'], 10, 25, 45)
        pdf.set_y(current_y + 8)
...
def extract_face_encoding(image_path):
    try:
        ...
        print(f"Face encoding error for {image_path}: {e}")
        return None
=======
...
def analyze_aadhaar_with_gemini(image_path):
    try:
        ...
        prompt = """
<<<<<<< HEAD
        Analyze the provided Aadhaar card image meticulously. Extract the following information and return it as a single, clean JSON object with these exact keys: 'name', 'relation' (like S/O, D/O, W/O from the address line or near the name), 'dob' (in YYYY-MM-DD format if possible, otherwise as text), 'address', 'gender', 'contact' (the mobile number if visible), and 'aadhaar_no'. If a specific field is not clearly visible or present, use the string "N/A" as its value. Do not include any text outside the JSON object.
        Example response: {"name": "Akash Kumar", "relation": "S/O: Ramesh Kumar", "dob": "15-05-1990", "address": "123, Main Road, Bhubaneswar, Odisha, 751001", "gender": "Male", "contact": "N/A", "aadhaar_no": "1234 5678 9012"}
        """
        response = model.generate_content([prompt, img])
        text_response = response.text.strip().replace("```json", "").replace("```", "").strip()
        
        # Find the JSON part even if there is other text
...
def analyze_blood_report_with_gemini(image_path):
    ...
    try:
        img = Image.open(image_path)
        prompt = "Analyze the provided blood report and identify the blood group. Return a JSON object with a single key 'blood_group'. Example: {\"blood_group\": \"O+\"}"
        response = model.generate_content([prompt, img])
        text_response = response.text.strip().replace("```json", "").replace("```", "").strip()
...
<<<<<<< HEAD
        ...
        if send_otp_email(email, otp):
            conn.execute('INSERT INTO users (name, email, password_hash, user_type, otp, otp_expiry) VALUES (?, ?, ?, ?, ?, ?)',
                         (name, email, password_hash, user_type, otp, otp_expiry))
            conn.commit()
            conn.close()
            flash('A verification OTP has been sent to your email.', 'success')
            return redirect(url_for('verify_otp', email=email))
        else:
...
def user_dashboard():
    ...
    conn = get_db_connection()
<<<<<<< HEAD
    # MODIFICATION: Join with users table to get user's name for display
    profile_query = '''
        ...
        try:
            for key in doc_keys:
                ...
                if not base64_data or not base64_data.startswith('data:image'):
                    # Check if it was a direct file upload instead (fallback)
=======
        try:
            for key in doc_keys:
                base64_data = request.form.get(key)
...
<<<<<<< HEAD
            ...
            if not blood_data: # Can be optional, will store N/A if fails
                blood_data = {'blood_group': 'N/A'}

            face_encoding_data = extract_face_encoding(full_paths['recent_photo'])
            if not face_encoding_data:
                return jsonify({'success': False, 'message': 'Could not detect a face in your photo. Please use a clear, front-facing picture.'}), 400

            # Database Update
            conn = get_db_connection()
            conn.execute('''
                UPDATE user_profiles SET 
                is_verified = 1, recent_photo = ?, dental_xray = ?, aadhaar_card = ?,
                blood_group_report = ?, face_encoding = ?, aadhaar_name = ?, aadhaar_relation = ?,
                aadhaar_dob = ?, aadhaar_address = ?, aadhaar_gender = ?, aadhaar_contact = ?,
                aadhaar_no = ?, blood_group = ? WHERE user_id = ?
            ''', (
                saved_filenames['recent_photo'], saved_filenames['dental_xray'], saved_filenames['aadhaar_card'],
                saved_filenames['blood_group_report'], face_encoding_data, aadhaar_data.get('name', 'N/A'),
                aadhaar_data.get('relation', 'N/A'), aadhaar_data.get('dob', 'N/A'), aadhaar_data.get('address', 'N/A'),
                aadhaar_data.get('gender', 'N/A'), aadhaar_data.get('contact', 'N/A'),
                aadhaar_data.get('aadhaar_no', 'N/A'), blood_data.get('blood_group', 'N/A'), user_id
            ))
            conn.commit()
            conn.close()
...
<<<<<<< HEAD
    ...
    cursor = conn.cursor()
    cursor.execute('''
        INSERT INTO reports (reporter_id, status, photo_path, name, age, hair_color, eye_color, skin_color, 
                                identity_marks, height, weight, aadhaar, contact_no, clothing_description, 
                                other_details, last_seen_date, last_seen_time, last_seen_location)
        VALUES (:reporter_id, :status, :photo_path, :name, :age, :hair_color, :eye_color, :skin_color, 
                :identity_marks, :height, :weight, :aadhaar, :contact_no, :clothing_description, 
                :other_details, :last_seen_date, :last_seen_time, :last_seen_location)
    ''', report_data)
...
# --- UPGRADED OFFICIAL DASHBOARD ROUTE (Unchanged) ---
=======
>>>>>>> ab532e4884dc155f32e60aec0f68754d7ca19613
...
<<<<<<< HEAD
        ...
        try:
            ...
            if aadhaar_no_input:
                search_aadhaar_numbers.add(clean_aadhaar_number(aadhaar_no_input))
=======
        try:
...
<<<<<<< HEAD
                encoding_str = extract_face_encoding(p_path)
                if encoding_str:
                    input_face_encodings.append(np.array(json.loads(encoding_str)))
                else:
                    flash('Could not detect a face in the provided Person/Dead Body image.', 'warning')
...
<<<<<<< HEAD
                encoding_str = extract_face_encoding(a_path)
                if encoding_str:
                    input_face_encodings.append(np.array(json.loads(encoding_str)))
                else:
                    flash('Could not detect a face in the provided Aadhaar image.', 'warning')
=======
...
<<<<<<< HEAD
                        flash(f"Extracted Aadhaar number '{cleaned_num}' from image for matching.", 'info')
...
<<<<<<< HEAD
            ...
            for profile in all_profiles:
                is_match = False
                if name_input and profile['aadhaar_name'] and name_input.lower() in profile['aadhaar_name'].lower():
                    is_match = True
                if not is_match and search_aadhaar_numbers and profile['aadhaar_no']:
                    db_aadhaar = clean_aadhaar_number(profile['aadhaar_no'])
                    if db_aadhaar and any(num == db_aadhaar for num in search_aadhaar_numbers):
                        is_match = True
                if not is_match and input_face_encodings and profile['face_encoding']:
                    db_encoding = np.array(json.loads(profile['face_encoding']))
                    matches = face_recognition.compare_faces(input_face_encodings, db_encoding, tolerance=0.6)
                    if any(matches): is_match = True
            ...
            if not match_result:
                ...
                flash(f"No profile match. Now searching {len(all_reports)} reports...", "info")
                
                for report in all_reports:
                    is_match = False
                    if name_input and report['name'] and name_input.lower() in report['name'].lower():
                        is_match = True
                    if not is_match and search_aadhaar_numbers and report['aadhaar']:
                        db_aadhaar = clean_aadhaar_number(report['aadhaar'])
                        if db_aadhaar and any(num == db_aadhaar for num in search_aadhaar_numbers):
                            is_match = True
                    if not is_match and input_face_encodings and report['photo_path']:
                        report_photo_path = os.path.join(app.config['UPLOAD_FOLDER'], report['photo_path'])
                        if os.path.exists(report_photo_path):
                            report_encoding_str = extract_face_encoding(report_photo_path)
                            if report_encoding_str:
                                report_encoding = np.array(json.loads(report_encoding_str))
                                matches = face_recognition.compare_faces(input_face_encodings, report_encoding, tolerance=0.6)
                                if any(matches): is_match = True
...
<<<<<<< HEAD
            ...
            return render_template('official_dashboard.html', 
                                   ...
                                   no_match_found=not match_result,
                                   uploaded_xray_filename=uploaded_xray_filename,
...
# --- MODIFICATION START: New route for the official list page ---
@app.route('/list_official')
def list_official():
    """
    Renders the list of all reports specifically for official users.
    This corresponds to the 'List' button in the official dashboard.
    """
    if 'user_id' not in session or session.get('user_type') != 'official':
        return redirect(url_for('login'))
    ...
    # <--- ADD THIS BLOCK TO FETCH THE USER'S DATA
    # This is required because your template needs the `current_user` variable.
    current_user = conn.execute('SELECT * FROM users WHERE id = ?', (session['user_id'],)).fetchone()
    
    reports = conn.execute('SELECT * FROM reports ORDER BY report_timestamp DESC').fetchall()
    conn.close()
    
    # Note: You will need to create a `list_official.html` template file.
    # It can be a copy of `list.html` adapted for the officials' view.
    
    # <--- UPDATE THIS LINE TO PASS THE NEW VARIABLE TO THE TEMPLATE
